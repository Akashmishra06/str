            ...
            for stock in subPortfolio:
                ...
                try:
                    df_1d, candle_flag_1d, last_candle_time_1d = OHLCDataFetch(
                        ...
                        150, self.candle_1d[stock]['df'], self.stockDict[stock].stockLogger
                    )
                ...
                try:
                    ...
                    rsi_time_period = int(config.get('technicalIndicatorParameters', 'rsiTimePeriod'))
                    df_1d['rsi'] = talib.RSI(df_1d['c'], timeperiod=rsi_time_period)
                    df_1d['macd'], df_1d['macd_signal'], df_1d['macd_hist'] = talib.MACD(
                        df_1d['c'],fastperiod=12,slowperiod=26,signalperiod=9)

                    self.stockDict[stock].stockLogger.info(
                        f"[1d] => Close: {df_1d.at[df_1d.index[-1], 'c']}, "
                        f"RSI: {df_1d.at[df_1d.index[-1], 'rsi']}, "
                        f"MACD: {df_1d.at[df_1d.index[-1], 'macd']}, "
                        f"MACD Signal: {df_1d.at[df_1d.index[-1], 'macd_signal']}, "
                        f"MACD Histogram: {df_1d.at[df_1d.index[-1], 'macd_hist']}"
                    )
                except Exception as e:
                    self.stockDict[stock].stockLogger.error(f"Error processing indicators for {stock}: {e}")
                    continue

                if not (time(15, 15) <= currentTime <= time(15, 30)):
                    continue

                # Exit Conditions
                if not self.stockDict[stock].openPnl.empty:
                    for index, row in self.stockDict[stock].openPnl.iterrows():
                        if (df_1d.at[df_1d.index[-1], 'rsi'] < (int(config["inputParameters"]["rsiEntryExit"])) and (row['EntryTime'].date() != datetime.now().date()):
                            exitType = "TargetHit"
                            self.stockDict[stock].exitOrder(index, self.idMap[row['Symbol']], row['CurrentPrice'], exitType)
                self.stockDict[stock].pnlCalculator()

                # Entry Conditions
                if self.stockDict[stock].openPnl.empty: #rsiEntryExit
                    if (df_1d.at[df_1d.index[-1], 'rsi'] > (int(config["inputParameters"]["rsiEntryExit"])) and (df_1d.at[df_1d.index[-1], 'macd'] > df_1d.at[df_1d.index[-1], 'macd_signal'] )and (df_1d.at[df_1d.index[-2], 'macd'] < df_1d.at[df_1d.index[-2], 'macd_signal']):
                        ...
                        infoMessage(algoName=algoName, message=f'Entry BUY: {stock} @ {currentPrice}, Qty- {quantity}')
