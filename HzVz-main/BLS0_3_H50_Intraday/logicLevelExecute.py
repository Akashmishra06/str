        ...
        try:
            ...
            while True:
                ...
                for subPortfolio in portfolio:
                    if (current_time >= time(10,15) and current_time <= time(15,20) and current_hour in hourIn and current_minute > 14 and current_minute < 16) or (current_time >= time(15,15)):
                ...
                if (readJson("algoStart") == True) or ((time(9, 20) < currentTime < time(9, 50)) or (time(15, 21) < currentTime < time(15, 29))):
                    try:
        ...
        try:
            ...
            for stock in subPortfolio:
                try:
                    ...
                    self.stockDict[stock].stockLogger.error(f"Error fetching current price for {stock}: {e}")
                    continue

                try:
                    df_1d, candle_flag_1d, last_candle_time_1d = OHLCDataFetch(stock, currentDatetime.timestamp(),
                        self.candle_1d[stock]['last_candle_time'], 'd', 150, self.candle_1d[stock]['df'], self.stockDict[stock].stockLogger)
                ...
                try:
                    df_1Min, candle_flag_1Min, last_candle_time_1Min = OHLCDataFetch(stock, currentDatetime.timestamp(),
                        self.candle_1Min[stock]['last_candle_time'], 1, 5, self.candle_1Min[stock]['df'], self.stockDict[stock].stockLogger)
                ...
                try:
                    ...
                    self.stockDict[stock].stockLogger.info(f"[1d] => Close: {df_1d.at[df_1d.index[-1], 'c']}")
                except Exception as e:
                    self.stockDict[stock].stockLogger.error(f"Error processing RSI for {stock}: {e}")
                    continue
                ...
                try:
                    ...
                    if not self.stockDict[stock].openPnl.empty:
                        for index, row in self.stockDict[stock].openPnl.iterrows():
                            if row['EntrycurrentDate'] != currentDate and time(row['curTime']) >= time(15, 14) and currentTime >= time(9, 16) and rsi_previous < int(config["inputParameters"]["EntryTimeRsi"]):
                                self.breakEven[stock] = False
                            ...
                            elif currentTime >= time(15, 15):

                                if row['EntrycurrentDate'] == currentDate and time(row['curTime']) < time(14, 30) and rsi_current < int(config["inputParameters"]["EntryTimeRsi"]):
                                    self.breakEven[stock] = False
                                    writeJson(f"breakEven{stock}", self.breakEven[stock])
                                    self.stockDict[stock].exitOrder(index, self.idMap[row['Symbol']], row['CurrentPrice'], "DailyIntradayExit")

                                elif currentPrice < row['EntryPrice'] and rsi_current < int(config["inputParameters"]["breakevenExitRsi"]):
                    ...
                    if self.stockDict[stock].openPnl.empty and nowTotalTrades < TotalTradeCanCome:
                        ...
                        entry_condition_1 = (time(15, 15) <= currentTime < time(15, 20)) and (rsi_current > int(config["inputParameters"]["EntryTimeRsi"])) and (rsi_current > rsi_previous)
                        ...
                        if entry_condition_1 or entry_condition_2:
                            self.breakEven[stock] = False
                            writeJson(f"breakEven{stock}", self.breakEven[stock])
                            self.stockDict[stock].entryOrder(self.idMap[stock], stock, currentPrice, quantity, "BUY", {"EntrycurrentDate": currentDate, "prev_day_c": prev_day_c, "curTime": currentTime})
