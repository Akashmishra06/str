        ...
        try:
            ...
            for stock in subPortfolio:
                ...
                try:
                    ...
                    # if not ((current_time >= time(10,15) and current_time < time(15,20) and current_hour in hourIn and current_minute >= 1 and current_minute < 50) or (current_time >= time(15,15))):
                    #     continue

                    if not self.stockDict[stock].openPnl.empty:
                        for index, row in self.stockDict[stock].openPnl.iterrows():
                            if currentTime >= time(10, 15):
                                ...
                                if df_weekly.at[df_weekly.index[-2], 'rsi'] < 40:
                                    ...
                                    if PnLL > 0:
                                        ...
                                        infoMessage(algoName=algoName, message=f"TotalProfitAmount: {round(LossAmount)}")
                                ...
                                elif currentPrice < row['EntryPrice'] and rsi_current < 61:
                                    self.breakEven[stock] = True
                                    writeJson(f"breakEven{stock}", self.breakEven[stock])
                                ...
                                if self.breakEven[stock] and currentPrice > row['EntryPrice']:
                                    if df_1d.at[df_1d.index[-1], 'rsi'] < 61: #int(config["inputParameters"]["EntryTimeRsi"])
