...
# def readJsonEntry(key=None):
#     file_path = f"{fileDir['jsonValue']}/WeeklyEntryRange.json"
...
# def writeJsonEntryValue(key, value):

#     file_path = f"{fileDir['jsonValue']}/WeeklyEntryRange.json"
#     jsonDict = readJsonEntry()
#     jsonDict[key] = value
#     with open(file_path, 'w') as json_file:
                ...
                try:
                    ...
                    if self.stockDict[stock].openPnl.empty:
                        ...
                        if current_time > time(14,40):
                            ...
                            if weeklyRange == True and (df_1d.at[df_1d.index[-1], 'rsi'] >  df_1d.at[df_1d.index[-2], 'rsi']):
                            ...
                            if weeklyRange == True and (df_1d.at[df_1d.index[-1], 'rsi'] > 50):
                                weeklyRsi = round(df_weekly.at[df_weekly.index[-2], 'rsi'], 2)
                                self.breakEven[stock] = False
                                dailyRsi = round(df_1d.at[df_1d.index[-1], 'rsi'], 2)
                                # quantity = (int(config["inputParameters"]["amountpertrade"]) // currentPrice)
