            ...
            for stock in subPortfolio:
                ...
                if not self.stockDict[stock].openPnl.empty:
                    for index, row in self.stockDict[stock].openPnl.iterrows():
                        if (df_1d.at[df_1d.index[-1], 'rsi'] < 30) and (row['EntryTime'].date() != datetime.now().date()):
                            exitType = "TargetHit"
                ...
                if self.stockDict[stock].openPnl.empty:
                    if (df_1d.at[df_1d.index[-1], 'rsi'] > 30) and (df_1d.at[df_1d.index[-1], 'macd'] > df_1d.at[df_1d.index[-1], 'macd_signal'] )and (df_1d.at[df_1d.index[-2], 'macd'] < df_1d.at[df_1d.index[-2], 'macd_signal']):
